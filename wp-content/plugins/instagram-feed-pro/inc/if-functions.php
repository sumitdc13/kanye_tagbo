...
function display_instagram( $atts = array() ) {
	...
	if ( $settings['caching_type'] === 'permanent' && empty( $settings['doingModerationMode'] ) ) {
		...
		if ( $instagram_feed->need_to_start_cron_job() ) {
			...
			SB_Instagram_Cron_Updater_Pro::do_single_feed_cron_update( $instagram_feed_settings, $to_cache, $atts, false );
...
function sbi_get_feed_template_part( $part, $settings = array() ) {
	...
	if ( $part === 'header' ) {
	    if ( isset( $settings['generic_header'] ) ) {
		    $file = trailingslashit( SBI_PLUGIN_DIR ) . 'templates/header-generic.php';
	    } else {
		    if ( $settings['headerstyle'] !== 'boxed' ) {
			    $file = trailingslashit( SBI_PLUGIN_DIR ) . 'templates/header.php';
		    } else {
			    $file = trailingslashit( SBI_PLUGIN_DIR ) . 'templates/header-boxed.php';
		    }
        }
	} elseif ( $part === 'item' ) {
